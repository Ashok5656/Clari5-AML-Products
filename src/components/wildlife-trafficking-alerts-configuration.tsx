...
interface WildlifeTraffickingAlertsConfigurationProps {
  breadcrumbs?: any[];
...
const rawRules = [
  "Individual/Firm involved in skin and leather goods trading with a high credit turnover and incoming transfers from unrelated parties.",
  "Industries like forestry resources, wildlife trade, traditional medicine, and manufacturing of items such as bone carvings or faux fur etc.",
  "Entities involved in wildlife-related industries such as private zoos, breeders, pet stores, safari companies, aquariums, and wildlife reserves.",
  "Individuals or businesses with adverse media exposure, or identified connections to known wildlife traffickers or organized crime networks.",
  "A large volume of cash deposits and withdrawals in accounts of state/central government officials who are working in forest department such as forest officers or rangers, may indicate involvement in bribery, corruption, for facilitation of wildlife trafficking.",
  "Charitable trusts or societies operating cattle shelters without registration with governing or regulatory bodies like NITI Aayog or the Animal Welfare Board of India.",
  "Accounts receiving third-party wire transfers or huge cash deposits, or showing withdrawals by known wildlife poachers and traffickers.",
  "Large wire transfers between wildlife farms / firms in unrelated or inconsistent industries.",
  "Transaction descriptions or communication details (e.g., email addresses, remittance information, and payment references) that include terms associated with illegal wildlife trade, such as endangered species or animal parts (e.g., ivory, tusks, scales, shark fins, tortoises, or geckos).",
  "A large number of small-value UPI (Unified Payments Interface) or IMPS (Immediate Payment Service) credits and deposits linked to the sale of Ayurvedic health products, especially when the amounts are below typical transaction thresholds that would raise suspicion for large-scale purchases.",
  "Frequent or unusual card/ATM usage in locations that are either the source or destination points for wildlife trafficking, or in towns and countries historically known for illegal wildlife trade routes.",
  "Geographical proximity of slaughterhouses to designated wildlife zones or sanctuaries, especially when these facilities are located in regions known for illegal wildlife trade.",
  "Unusual patterns of activity at recreational businesses (such as resorts, water sports activities, selling of souvenirs and tourism-related services) located within or near wildlife sanctuaries or protected areas, particularly if they are in remote regions with limited law enforcement or oversight."
];

const generateRulesData = (): AlertRule[] => {
  return rawRules.map((desc, index) => {
    let parameters = "Customer Type: Individual, Transaction Volume: > 50000, Location Risk: High";
    
    if (desc.includes("skin and leather goods trading")) {
       parameters = "Minimum number of Remitter: 5, Minimum Credit Turnover Amount: > 1000000, Nature of Business: Skin and Leather Goods Trading";
    }

    return {
      id: `WT-${(index + 1).toString().padStart(3, '0')}`,
      description: desc,
      severity: "High",
      status: "Active",
...
export function WildlifeTraffickingAlertsConfiguration({ breadcrumbs, onBreadcrumbNavigate }: WildlifeTraffickingAlertsConfigurationProps) {
