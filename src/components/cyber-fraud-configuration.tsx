...
interface CyberFraudConfigurationProps {
  breadcrumbs?: any[];
...
const rawRules = [
  "Change in IP/MAC address after opening of account of characteristics described in scenario 16.",
  "Large velocity of small value credits with payment instrument predominantly UPI/IMPS exceeding declared turnover.",
  "Large velocity of small value debits with payment instrument predominantly UPI/IMPS exceeding declared turnover.",
  "Large velocity of small value credits with payment instrument predominantly UPI/IMPS exceeding declared turnover followed by large velocity of small value debits with payment instrument predominantly UPI/IMPS exceeding declared turnover.",
  "Large velocity of small value credits with payment instrument predominantly UPI/IMPS exceeding declared turnover followed by cash withdrawals in ATMs abroad (predominantly UAE, Hong Kong, Bangkok).",
  "Large velocity of small value credits with payment instrument predominantly UPI/IMPS exceeding declared turnover followed by cross border remittances.",
  "Large velocity of small value credits with payment instrument predominantly UPI/IMPS exceeding declared turnover followed by conversion into virtual assets.",
  "Large velocity of small value credits with payment instrument predominantly UPI/IMPS exceeding declared turnover, followed by large velocity of small value debits with payment instrument predominantly UPI/IMPS exceeding declared turnover, followed by cash withdrawals in ATMs abroad (predominantly UAE, Hong Kong, Bangkok), followed by cross border remittances, followed by conversion into virtual assets, in a dormant / newly opened account.",
  "Large velocity of small value credits with payment instrument predominantly UPI/IMPS exceeding declared turnover, followed by large velocity of small value debits with payment instrument predominantly UPI/IMPS exceeding declared turnover, followed by cash withdrawals in ATMs abroad (predominantly UAE, Hong Kong, Bangkok), followed by cross border remittances, followed by conversion into virtual assets, in a dormant / newly opened account observed with Multiple Customers Working Together - Multiple accounts opened with common email addresses, mobile number, PAN, or suspicious email addresses (emails not compatible with accountholder)",
  "Occurrence of scenarios 3 with 4/5/6/7/8",
  "Current accounts with access to facilities for creation of virtual accounts / multiple virtual payment addresses (VPAs)",
  "Current accounts with facilities for bulk pay-out services",
  "Request for change in registered mobile number (RMN) / email address with seen with scenarios 10/11",
  "Accounts accessed from suspicious IP addresses (multiple IP addresses, VPN, foreign IP addresses, repository of blacklisted IP addresses) seen with scenarios 10/11/12.",
  "Multiple accounts being accessed from common IP seen with scenario 10/11/12/13.",
  "Scenario 1/2/3/4/5/6/7/8/9 observed with scenario 10/11/12/13/14.",
  "Accounts opened through simplified KYC process, involving video-based customer identification process (V-CIP), remotely at branches located in states different from the state of the victim’s location.",
  "Accounts of characteristics described in scenario 16 opened from areas known as high-risk for cyber frauds.",
  "Request for change in registered mobile number (RMN) / email address with seen with scenarios 16/17/18.",
  "Accounts accessed from suspicious IP addresses (multiple IP addresses, VPN, foreign IP addresses, repository of blacklisted IP addresses) seen with scenarios 16/17/18/19.",
  "Multiple accounts being accessed from common IP seen with scenario 16/17/18/19/20.",
  "Scenarios 1, 2, 3, 7 seen in accounts opened with the characteristics described in scenarios 16/17/18/19/20/21.",
  "Scenario 1 followed by ATM withdrawals which form S% of total withdrawals in accounts with characteristics described in scenarios 16/17/18/19/20/21/22."
];

const generateRulesData = (): AlertRule[] => {
  return rawRules.map((desc, index) => {
    let parameters = "Transaction Velocity: High, Channel: Digital, Risk Score: > 85";
    
    if (desc.includes("Change in IP/MAC address")) {
       parameters = "Monitoring Period: From Date to To Date";
    }

    return {
      id: `CF-${(index + 1).toString().padStart(3, '0')}`,
      description: desc,
      severity: "High",
      status: "Active",
      category: desc.toLowerCase().includes("upi") ? "UPI Activity" : desc.toLowerCase().includes("ip") ? "Digital Footprint" : "Account Activity",
...
export function CyberFraudConfiguration({ breadcrumbs, onBreadcrumbNavigate }: CyberFraudConfigurationProps) {
  ...
  if (currentView === "view" && selectedRule) {
    return (
      <div className="flex flex-col h-full bg-white dark:bg-gray-900/50 p-4">
        {/* Header */}
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-gray-900/50 p-4">
      ...
      <div className="flex-none pb-4 flex items-center justify-between">
         ...
         <div className="flex gap-2">
            <Button variant="outline" onClick={onCancel} className="gap-2 bg-white text-gray-700 hover:bg-gray-50 border-gray-300">
              Cancel
            </Button>
            <Button onClick={() => onSave(formData)} className="gap-2 bg-[#2A53A0] hover:bg-[#2A53A0]/90 text-white">
