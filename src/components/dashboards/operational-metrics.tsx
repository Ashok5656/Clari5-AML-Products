...
export function OperationalMetrics({ breadcrumbs, onBreadcrumbNavigate }: OperationalMetricsProps) {
  ...
  // 8. Alerts
  const alerts = [
    { id: 1, type: "success", title: "ACHIEVEMENT", time: "5 min ago", message: "Priya Sharma completed 20 cases today" },
    { id: 2, type: "info", title: "INFO", time: "18 min ago", message: "2 cases escalated by Amit Patel" },
    { id: 3, type: "warning", title: "WARNING", time: "26 min ago", message: "SLA breach for case AML-4521" },
    { id: 4, type: "error", title: "ALERT", time: "45 min ago", message: "New high-priority alert assigned to Team Alpha" },
    { id: 5, type: "success", title: "ACHIEVEMENT", time: "1 hour ago", message: "Team achieved 98% QA pass rate this week" },
  ];
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-4">
         ...
         <Card className="lg:col-span-7 bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 shadow-sm">
            <div className="p-6">
               ...
               <ResponsiveContainer width="100%" height={250} id="workload-dist-container" key="workload-dist-container">
                  <BarChart data={workloadData} layout="vertical" barSize={16} id="workload-dist-chart" key="workload-dist-chart">
                     <CartesianGrid strokeDasharray="3 3" horizontal={true} vertical={false} opacity={0.3} key="workload-grid" />
                     <XAxis type="number" fontSize={11} key="workload-xaxis" />
                     <YAxis dataKey="name" type="category" width={90} fontSize={11} tick={{fill: '#4b5563'}} key="workload-yaxis" />
                     <Tooltip cursor={{fill: 'transparent'}} contentStyle={{borderRadius: '8px', fontSize: '12px'}} key="workload-tooltip" />
                     <Legend wrapperStyle={{fontSize: '11px', paddingTop: '10px'}} key="workload-legend" />
                     <Bar dataKey="open" name="Open" stackId="a" fill="#2A53A0" radius={[0,0,0,0]} id="workload-bar-open" key="workload-bar-open" />
                     <Bar dataKey="inProgress" name="In Progress" stackId="a" fill="#eab308" radius={[0,0,0,0]} id="workload-bar-progress" key="workload-bar-progress" />
                     <Bar dataKey="pending" name="Pending Info" stackId="a" fill="#64748b" radius={[0,4,4,0]} id="workload-bar-pending" key="workload-bar-pending" />
                  </BarChart>
               </ResponsiveContainer>
            </div>
         </Card>
                  ...
                  <table className="w-full text-xs">
                     ...
                     <tbody className="space-y-1">
                        {sortedHeatmap.map((row) => (
                           <tr key={`heatmap-row-${row.priority}`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            ...
            <div className="p-6">
               ...
               <div className="divide-y divide-gray-100">
                  {leaderboardData.map((item) => (
                     <div key={`leaderboard-${item.name.replace(/\s+/g, '-')}`} className="flex items-center justify-between p-3 hover:bg-gray-50 transition-colors rounded-lg">
                        <div className="flex items-center gap-3">
         ...
         <Card className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 shadow-sm">
            <div className="p-6">
               ...
               <ResponsiveContainer width="100%" height={200} id="sla-breach-analysis-container" key="sla-breach-analysis-container">
                  <ComposedChart data={breachData} id="sla-breach-analysis-chart" key="sla-breach-analysis-chart">
                     <CartesianGrid strokeDasharray="3 3" opacity={0.3} vertical={false} key="sla-breach-grid" />
                     <XAxis dataKey="date" tick={{fontSize: 10}} tickMargin={10} interval={2} key="sla-breach-xaxis" />
                     <YAxis yAxisId="left" tick={{fontSize: 10}} width={20} key="sla-breach-yaxis-left" />
                     <YAxis yAxisId="right" orientation="right" tick={{fontSize: 10}} width={20} unit="%" key="sla-breach-yaxis-right" />
                     <Tooltip contentStyle={{borderRadius: '8px', fontSize: '12px'}} key="sla-breach-tooltip" />
                     <Bar yAxisId="left" dataKey="count" name="Breach Count" fill="#ea580c" barSize={10} radius={[2,2,0,0]} id="sla-breach-bar" key="sla-breach-bar" />
                     <Line yAxisId="right" type="monotone" dataKey="rate" name="Breach Rate %" stroke="#ef4444" strokeWidth={2} dot={false} id="sla-breach-line" key="sla-breach-line" />
                  </ComposedChart>
               </ResponsiveContainer>
            ...
            <div className="p-6">
               ...
               <div className="divide-y divide-gray-100">
                  {atRiskItems.map((item) => (
                     <div key={`at-risk-${item.id}`} className="flex items-center justify-between p-3 hover:bg-gray-50 transition-colors rounded-lg">
                        <div className="flex items-center gap-3">
               ...
               <div className="flex flex-col md:flex-row gap-6">
                  <div className="flex-1 flex flex-col items-center">
                     <div className="relative h-32 w-32">
                        <ResponsiveContainer width="100%" height="100%" id="qa-pie-container" key="qa-pie-container">
                           <PieChart id="qa-pie-chart" key="qa-pie-chart">
                              <Pie 
                                 data={qaPieData} 
                                 innerRadius={40} 
                                 outerRadius={55} 
                              ...
                              >
                                 {qaPieData.map((entry) => (
                                    <Cell key={`qa-pie-cell-${entry.name}`} fill={entry.color} />
                                 ))}
                              </Pie>
                           </PieChart>
                        </ResponsiveContainer>
                        <div className="absolute inset-0 flex items-center justify-center">
                           <span className="text-xl font-bold text-gray-900 dark:text-white">91%</span>
                        </div>
                     </div>
                  ...
                  <div className="flex-1">
                     ...
                     <ResponsiveContainer width="100%" height={120} id="qa-error-dist-container" key="qa-error-dist-container">
                        <BarChart data={qaErrorData} id="qa-error-dist-chart" key="qa-error-dist-chart">
                           <CartesianGrid strokeDasharray="3 3" vertical={false} opacity={0.3} key="qa-error-grid" />
                           <XAxis dataKey="name" tick={{fontSize: 9}} interval={0} key="qa-error-xaxis" />
                           <YAxis hide key="qa-error-yaxis" />
                           <Bar dataKey="value" fill="#d97706" radius={[4,4,0,0]} barSize={20} id="qa-error-bar" key="qa-error-bar" />
                        </BarChart>
                     </ResponsiveContainer>
                  </div>
               </div>
         ...
         <Card className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 shadow-sm">
            <div className="p-6">
               ...
               <div className="space-y-4 max-h-[200px] overflow-y-auto pr-2 no-scrollbar">
                  {alerts.map((alert) => (
                     <div key={`alert-msg-${alert.id}`} className="flex gap-3">
                        <div className={cn("p-1.5 rounded h-fit", 
                           ...
                           {alert.type === 'success' ? <CheckCircle className="size-4" /> : 
                            ...
                            alert.type === 'error' ? <ShieldAlert className="size-4" /> : <Bell className="size-4" />}
                        </div>
                        <div className="flex-1">
                           <div className="flex justify-between items-center mb-0.5">
                              <span className="text-[10px] font-bold uppercase tracking-wider">{alert.title}</span>
         ...
         <div className="p-6 border-b border-gray-200 dark:border-gray-800 flex justify-between items-center">
            ...
            <div className="flex gap-2">
               <div className="relative">
                  ...
                  <input type="text" placeholder="Search team member..." className="pl-9 pr-4 py-1.5 text-xs border border-gray-200 rounded-md bg-gray-50 focus:bg-white transition-all w-48 focus:outline-none" />
               </div>
            </div>
         </div>
         <div className="overflow-x-auto">
            <table className="w-full text-xs text-left">
               <thead className="bg-gray-50 dark:bg-gray-800/50 text-gray-500 font-medium">
                  <tr>
                     <th className="p-4"><SortableHeader column="name" label="Member Name" sortConfig={detailedConfig} onSort={sortDetailed} /></th>
                     <th className="p-4"><SortableHeader column="status" label="Status" sortConfig={detailedConfig} onSort={sortDetailed} /></th>
                     <th className="p-4"><SortableHeader column="queue" label="Queue" sortConfig={detailedConfig} onSort={sortDetailed} /></th>
                     <th className="p-4"><SortableHeader column="today" label="Today" sortConfig={detailedConfig} onSort={sortDetailed} /></th>
                     <th className="p-4"><SortableHeader column="week" label="Week" sortConfig={detailedConfig} onSort={sortDetailed} /></th>
                     <th className="p-4"><SortableHeader column="avgTat" label="Avg TAT" sortConfig={detailedConfig} onSort={sortDetailed} /></th>
                     <th className="p-4"><SortableHeader column="sla" label="SLA %" sortConfig={detailedConfig} onSort={sortDetailed} /></th>
                     <th className="p-4"><SortableHeader column="qa" label="QA Score" sortConfig={detailedConfig} onSort={sortDetailed} /></th>
                     <th className="p-4 text-right">Trend</th>
                  </tr>
               </thead>
               <tbody className="divide-y divide-gray-100">
                  {sortedDetailed.map((member) => (
                     <tr key={`member-row-${member.name.replace(/\s+/g, '-')}`} className="hover:bg-gray-50/80 transition-colors">
                        <td className="p-4 font-medium text-gray-900 dark:text-white flex items-center gap-2">
                           <div className="w-7 h-7 rounded-full bg-[#2A53A0] text-white flex items-center justify-center font-bold text-[10px] uppercase">
                        ...
                        <td className="p-4">{member.queue}</td>
                        <td className="p-4">{member.today}</td>
                        <td className="p-4">{member.week}</td>
                        <td className="p-4">{member.avgTat}</td>
                        <td className="p-4 font-bold">{member.sla}</td>
                        <td className="p-4">{member.qa}</td>
                        <td className="p-4 text-right">
                           {member.trend === 'up' ? <TrendingUp className="size-4 text-green-500 ml-auto" /> : 
                            member.trend === 'down' ? <TrendingDown className="size-4 text-red-500 ml-auto" /> : 
