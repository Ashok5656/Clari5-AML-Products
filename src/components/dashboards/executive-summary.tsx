    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <Card className="p-4 border-gray-200 dark:border-gray-800">
          ...
          <ResponsiveContainer width="100%" height={250} id="fraud-roi-container" key="fraud-roi-container">
            <BarChart data={monthlyFraudPrevention} id="fraud-roi-chart" key="fraud-roi-chart">
              <CartesianGrid strokeDasharray="3 3" opacity={0.1} key="fraud-roi-grid" />
              <XAxis dataKey="month" tick={{fontSize: 12, fill: "#9ca3af"}} key="fraud-roi-xaxis" />
              <YAxis tick={{fontSize: 12, fill: "#9ca3af"}} key="fraud-roi-yaxis" />
              <Tooltip 
                ...
                key="fraud-roi-tooltip"
              />
              <Bar dataKey="prevented" fill="#2A53A0" radius={[4, 4, 0, 0]} barSize={20} id="bar-prevented" key="bar-prevented" />
              <Bar dataKey="attempted" fill="#e5e7eb" radius={[4, 4, 0, 0]} barSize={20} id="bar-attempted" key="bar-attempted" />
            </BarChart>
          </ResponsiveContainer>
        </Card>
        ...
        <Card className="p-4 border-gray-200 dark:border-gray-800">
           ...
           <div className="space-y-6">
              {complianceScores.map((region) => (
                <div key={`regional-score-${region.region.replace(/\s+/g, '-')}`}>
       ...
       <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <Card className="p-4 border-gray-200 dark:border-gray-800">
             ...
             <ResponsiveContainer width="100%" height={200} id="cost-distribution-container" key="cost-distribution-container">
                <PieChart id="cost-distribution-chart" key="cost-distribution-chart">
                   <Pie
                      ...
                      paddingAngle={5}
                      dataKey="value"
                      id="cost-distribution-pie"
                      key="cost-distribution-pie"
                   >
                      {operationalEfficiency.map((entry) => (
                         <Cell key={`cost-cell-${entry.name.replace(/\s+/g, '-')}`} fill={entry.color} />
                      ))}
                   </Pie>
                   <Tooltip 
                      contentStyle={{ 
                      ...
                      key="cost-distribution-tooltip"
                   />
                </PieChart>
             </ResponsiveContainer>
             <div className="grid grid-cols-2 gap-2 mt-4">
                {operationalEfficiency.map((item) => (
                   <div key={`cost-legend-${item.name.replace(/\s+/g, '-')}`} className="flex items-center gap-2 text-xs">
          ...
          <Card className="lg:col-span-2 p-4 border-gray-200 dark:border-gray-800">
             ...
             <div className="space-y-4">
                {[
                   ...
                   <div key={`initiative-${item.name.replace(/\s+/g, '-')}`} className="flex items-center gap-4 p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
