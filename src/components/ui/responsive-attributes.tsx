    ...
    <div className={cn("relative w-full min-w-0", className)}>
      ...
      >
        {items.map((item, index) => (
          <div key={item.id || `measure-${index}`} className="whitespace-nowrap flex items-center">{item.content}</div>
        ))}
      </div>
      ...
      <div ref={containerRef} className="flex items-center gap-x-1 w-full overflow-hidden whitespace-nowrap text-sm">
        {items.slice(0, visibleCount).map((item, index) => (
          <div key={item.id || `visible-${index}`} className="contents">
                ...
                <Tooltip delayDuration={0}>
                  ...
                  <TooltipContent side="bottom" align="end" className="flex flex-col gap-2 p-3 bg-white text-gray-900 border border-gray-200 shadow-xl rounded-lg max-h-[300px] overflow-y-auto min-w-[200px] z-50">
                    {items.slice(visibleCount).map((item, index) => (
                      <div key={item.id || `more-${index}`} className="flex flex-col gap-1 pb-2 border-b border-gray-100 last:border-0 last:pb-0">
                        {item.content}
                      </div>
                    ))}
                  </TooltipContent>
